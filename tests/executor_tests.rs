...
fn test_compile_check_blocks_broken_go() {
    ...
    write_file(&go_file, "package main\n\nfunc main() {\n    fmt.Println(\"hello\")\n}\n");
    ...
    assert!(!output.status.success(), "go vet should fail on missing import");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("undefined: fmt"), "expected 'undefined: fmt'");
}
...
fn test_compile_check_blocks_broken_python() {
    ...
    write_file(&py_file, "import non_existent_module_xyz\nx = non_existent_module_xyz.func()\n");
    ...
    assert!(!output.status.success(), "py_compile should fail on missing import");
}
...
fn test_patch_normalize_preserves_escapes() {
    ...
    assert_eq!(backslash_count, 3, "original should have three backslashes (\\n, \\t, \\')");
    
    let normalized = original.replace(r#"\t"#, r#"\\t"#).replace(r#"\n"#, r#"\\n"#);
    assert!(normalized.contains(r#"\\t"#), "normalized must keep escaped tab");
}
...
fn test_autofix_after_write_file() {
    ...
    let _ = Command::new("go").args(&["mod", "init", "testmod"]).current_dir(&dir).output();

    let vet_fail = Command::new("go").args(&["vet", "."]).current_dir(&dir).output().unwrap();
    assert!(!vet_fail.status.success());
    
    let fixed = "package main\n\nimport \"fmt\"\n\nfunc main() {\n    fmt.Println(\"ok\")\n}\n";
    write_file(&go_file, fixed);
    
    let vet_ok = Command::new("go").args(&["vet", "."]).current_dir(&dir).output().unwrap();
...
fn test_go_vet_dot_slash_dot() {
    ...
    write_file(&go_file, "package lib\n\nfunc Hello() string { return \"hello\" }\n");
    
    let output = Command::new("go").args(&["vet", "./..."]).current_dir(&dir).output().unwrap();
    assert!(output.status.success(), "go vet ./... failed: {:?}", String::from_utf8_lossy(&output.stderr));
}
...
fn test_full_flow_compile_fail_then_autofix_then_test_pass() {
    ...
    let output_before = Command::new("go").args(&["test", "."]).current_dir(&dir).output().unwrap();
    assert!(!output_before.status.success());
    
    let src_file = dir.join("calc.go");
    write_file(&src_file, "package main\n\nfunc Add(a,b int) int { return a+b }\n");
    
    let output_after = Command::new("go").args(&["test", "."]).current_dir(&dir).output().unwrap();
    assert!(output_after.status.success(), "Stderr: {}", String::from_utf8_lossy(&output_after.stderr));
}
