...
echo "==> Bumping version in Cargo.toml and src/main.rs"
python3 - << 'PYEOF'
...
if cargo.exists():
    s = cargo.read_text()
    s2 = s.replace('version = "8.5.2"', 'version = "8.8.0"')
    cargo.write_text(s2)
...
if main.exists():
    s = main.read_text()
    s = s.replace("// src/main.rs  SEL Agent v8.5.1", "// src/main.rs  SEL Agent v8.8.0")
    s = s.replace("// src/main.rs  SEL Agent v8.5.2", "// src/main.rs  SEL Agent v8.8.0")
    main.write_text(s)

print("✅ version bump applied")
PYEOF

echo "==> Writing docs/SEL_AGENT_REFERENCE_v8.8.0.md"
cat > docs/SEL_AGENT_REFERENCE_v8.8.0.md << 'DOC_EOF'
...
cp docs/SEL_AGENT_REFERENCE_v8.8.0.md docs/SEL_AGENT_REFERENCE_latest.md
echo "✅ docs/SEL_AGENT_REFERENCE_v8.8.0.md written"
echo "✅ docs/SEL_AGENT_REFERENCE_latest.md updated"

echo "==> Updating CHANGELOG.md"
python3 - << 'PYEOF'
from pathlib import Path

entry = """## v8.8.0 — observability + dependency graph (2026-06-04)
...
Added dependency graph core model
Added Python / TypeScript / Go / Rust parsers
Added workspace graph builder
Added graph-aware context scoring
Notes
No benchmark regression detected in core replay gate
Next milestone: Pattern Library + Adaptive Repair Routing
"""
p = Path("CHANGELOG.md")
...
if "## v8.8.0 — observability + dependency graph (2026-06-04)" not in text:
...
echo " cargo test"
echo " bash scripts/regression_gate.sh core"
echo
echo "Then save the release:"
echo " git add Cargo.toml src/main.rs CHANGELOG.md docs/SEL_AGENT_REFERENCE_v8.8.0.md docs/SEL_AGENT_REFERENCE_latest.md"
echo " git commit -m "Release v8.8.0""
echo " git tag -a v8.8.0 -m "SEL Agent v8.8.0""
