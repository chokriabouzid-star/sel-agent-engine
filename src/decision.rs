...
pub fn evaluate_plan_risk(workspace: &Path, plan: &[Cmd]) -> PlanRiskReport {
        ...
        match cmd {
            Cmd::WriteFile { path, .. } => {
                ...
                if exists && is_test {
                    ...
                    report.reasons.push(format!(
                        "PLAN RISK: write_file targets existing source file '{}'  prefer patch_file for surgical fixes.",
                        path
                    ));
                }
            }
            Cmd::PatchFile { path, .. } => {
                ...
                if full.exists() && is_test_like_path(path) {
                    ...
                    report.estimated_risk += 0.50;
                    report.reasons.push(format!(
                        "PLAN RISK: patch_file targets existing test file '{}'  fix implementation files instead.",
                        path
                    ));
                }
            }
            Cmd::DeleteFile { path } => {
                ...
                report.reasons.push(format!(
                    "PLAN RISK: delete_file on '{}' is destructive and should be avoided unless strictly necessary.",
                    path
                ));
            }
            _ => {}
    ...
    if plan.len() >= 8 {
        ...
        report.reasons.push(format!(
            "PLAN RISK: plan has {} commands  this is unusually large and may indicate drift.",
            plan.len()
        ));
    }
